...
// this better versions have been introduced by Dr. mostafa saad


// For first solution O(nlogn) see approach 3: https://leetcode.com/problems/shortest-unsorted-continuous-subarray/solution/

class Solution {
public:
    int findUnsortedSubarray3(vector<int> &nums) {	// O(nlog(n)) time - O(n) space
...
//Src: https://leetcode.com/problems/shortest-unsorted-continuous-subarray/discuss/336218/C%2B%2B-O(n)-time-O(1)-space
/*
 ...
 */
    int findUnsortedSubarray2(vector<int> &nums) {	// O(n) time - O(1) space
